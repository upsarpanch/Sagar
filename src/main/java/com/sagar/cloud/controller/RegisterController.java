...
public class RegisterController {
	...
	String getRegister(@RequestParam String firstName, @RequestParam String lastName, @RequestParam String email,
			...
			@RequestParam String pinCode, @RequestParam String branchName, Model mv,RedirectAttributes redirectAttributes) {
		try {
			...
			if (user != null) {
				if (userRepository.findByEmail(email) != null) {
					redirectAttributes.addFlashAttribute("message", "Customer already register,please try new one!");
				} else {
					branchRepository.save(branch);
					userRepository.save(user);
					redirectAttributes.addFlashAttribute("message", "Customer Registration is done successfully!");
				}
